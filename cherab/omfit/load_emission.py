...
def load_emission(config, plasma, xr, yr, num):

    models    = []
    tmpmodel  = []
    em_plasma = np.zeros((num,num))
    ...
    for emission_instruction in config["plasma"]["emission_instructions"]:
        ...
        if emission_instruction["prof2D"] and config['plasma']['edge']['em2D']:
            ...
            for i, x in enumerate(xr):
                for j, y in enumerate(yr):
                    em_plasma[j,i] = em_plasma[j,i] + line_detail.emission(Point3D(x, 0.0, y), Vector3D(1.0, 0, 0),
